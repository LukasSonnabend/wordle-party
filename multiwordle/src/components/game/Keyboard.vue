...
<template>
  <div class="keyboard w-screen max-w-sm mt-2 h-full">
    <div class="flex justify-end w-full gap-0.5 h-1/4 lg:gap-1">
      <Key v-for="(letter) in letters.slice(0, 10)" :letter="letter" @click="$emit('letterEvent', letter)" :key="letter"/>
    </div>
    <div
      className="flex justify-end w-full my-2 h-1/4 gap-0.5 lg:gap-1 basis-auto"
    >
      <span className="half-key" />
      <Key v-for="(letter) in letters.slice(10, 19)" :letter="letter" @click="$emit('letterEvent', letter)" :key="letter"/>
      <span className="half-key" />
    </div>
    <div className="flex justify-center w-full gap-0.5 lg:gap-1 h-1/4">
      <Key letter="ENTER" @click="$emit('sendGuess')"/>
      <Key v-for="(letter) in letters.slice(19, 30)" :letter="letter" @click="$emit('letterEvent', letter)" :key="letter"/>
      <Key
        letter="DELET"
        classPass="min-w-12"
